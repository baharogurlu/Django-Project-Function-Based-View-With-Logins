...
mxEditor.prototype.addActions = function ()
{
	...
	this.addAction('delete', function(editor)
	{
		if (editor.graph.isEnabled())
		{
			var cell = editor.graph.getSelectionCells(); 
            for(var i = 0; i < cell.length; i++) {
            	var  sty=cell[i].style;
            	if(sty != null ){
                		var riskId =  cell[i].getAttribute('riskId', '');
                 
                		var surecAdimId =  cell[i].getAttribute('adimid', '');
                		 if(surecAdimId != null &&  surecAdimId != ""){
                			  $.ajax({
        							url:'./silSurecAdim?surecAdimId='+surecAdimId ,
        							async:false,
